...
const WelcomeView = ({ showWelcome, messages }) => {
  ...
  return (
    <div
      className={`animate-fadeIn2 absolute top-1/2 left-1/2 transform -translate-x-1/2 transition-all duration-500 ease-in-out text-center text-3xl font-bold text-gray-800 z-10 opacity-100 -translate-y-1/2`}
    >
      {isThinking ? (
        <>Thinking...</>
      ) : (
...
const ChatArea = ({ messages: initialMessages, className }) => {
  ...
  const updateThinkingMessage = (thinkingId, content) => {
    setMessages((prev) =>
      prev.map((msg) =>
  ...
  const sendMessage = async (customText = null) => {
    ...
    const positiveMessages = [
      "We’re here for you...", "Finding the right words...", "You’re not alone...",
      "Crafting a warm reply...", "Thinking of you...", "Help is on the way...",
      "Gathering some hope...",
    ];

    setMessages((prev) => [
      ...prev,
      { id: thinkingId, role: "thinking", color: "white", content: positiveMessages[0] },
    ]);

    const interval = setInterval(() => {
      messageIndex = (messageIndex + 1) % positiveMessages.length;
      updateThinkingMessage(thinkingId, positiveMessages[messageIndex]);
    }, 2500);
    ...
    try {
      ...
      setMessages((prev) => {
        const others = prev.filter((msg) => msg.id !== thinkingId);
        return [...others, { id: Date.now(), role: "assistant", content: data.content }];
      });
    } catch (error) {
      ...
      setMessages((prev) => {
        const others = prev.filter((msg) => msg.id !== thinkingId);
        return [...others, { id: Date.now(), role: "assistant", content: "Sorry, something went wrong!" }];
      });
    }
  };
    ...
    <div className={`relative h-full flex flex-col bg-gradient-to-b from-purple-50 to-white ${className}`} style={{ position: "relative" }}>
      <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-purple-50 to-transparent z-10">
        ...
        >
          ...
          <div className="text-white text-lg font-bold font-[Satoshi]">SOS Help</div>
        </button>
      </div>

      <div className="flex-1 overflow-y-auto pt-6 pb-8 space-y-4 md:px-16 lg:px-16 xl:px-32 relative">
        <WelcomeView showWelcome={showWelcome} messages={messages} />
      ...
      <div className="flex gap-2 flex-col">
        <SuggestionList suggestions={suggestions} onSelect={sendMessage} />
